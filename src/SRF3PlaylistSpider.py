...
class SRF3PlaylistSpider(DownloadSpider):
    ...
    def __init__(self, start_date=None, end_date=None, *args, **kwargs):
        ...
        Parameter:
            start_date (str, optional): Das Startdatum für den Abruf im Format `YYYY-MM-DD`.
                                        Wenn `None`, wird das heutige Datum verwendet.
            end_date (str, optional): Das Enddatum für den Abruf im Format `YYYY-MM-DD`.
                                      Wenn `None`, wird das heutige Datum verwendet.
        ...
        Löst aus:
            ValueError: Wenn `start_date` oder `end_date` ein ungültiges Format haben oder
                        wenn `end_date` vor `start_date` liegt.
        """
        ...
        if start_date:
            try:
                ...
                raise ValueError(f"Ungültiges start_date-Format: {start_date}, muss YYYY-MM-DD sein.")
        else:
            start = today
        ...
        if end_date:
            try:
                ...
                raise ValueError(f"Ungültiges end_date-Format: {end_date}, muss YYYY-MM-DD sein.")
        else:
            end = today
    ...
    def parse(self, response: HtmlResponse, **kwargs):
        ...
        if songs:
            filename = f"srf3_playlist_{filename_date_str}.json"
